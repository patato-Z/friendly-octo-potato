...
def get_model(model_name: str, model_type: str) -> nn.Module:
    """
...
class EarlyStopper(object):
    ...
    def __init__(self, patience, lr_min, optims, reduce_on_pleatue=False):
        ...
        self.lr_min = lr_min
        self.optims = optims
        self.reduce_on_pleatue = reduce_on_pleatue
    ...
    def stop_training(self, val_auc, weights):
        ...
        # elif 0:  # for debug only
            self.trial_counter += 1
            return False
        else:
            if self.reduce_on_pleatue:
                cur_lr = self.optims['optimizer_bb'].param_groups[0]['lr']
                logging.info(f"lr: {cur_lr} --> {cur_lr*0.1}")
                if cur_lr > self.lr_min + 1.e-8:
